...
Tinytest.add('MeteorFlux - AppState - Test internal _getObjNode function',
  function (test) {
    beforeEach();

    AppState._obj = { a: 'a' };
    test.equal(AppState._getObjNode(['a'], AppState._obj), 'a');

    AppState._obj = { a: { b: 'b' } };
    test.equal(AppState._getObjNode(['a'], AppState._obj), { b: 'b' });
    test.equal(AppState._getObjNode(['a', 'b'], AppState._obj), 'b');
  }
);
